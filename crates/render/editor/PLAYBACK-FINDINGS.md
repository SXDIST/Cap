...
| File | Purpose |
|------|---------|
| `crates/render/rendering/src/decoder.rs` | Main decoder interface, spawn_decoder() |
| `crates/media/media/video-decode/src/` | Platform-specific decoders |
| `crates/media/media/video-decode/src/macos.rs` | AVAssetReader hardware decoder |
| `crates/media/media/video-decode/src/ffmpeg.rs` | FFmpeg software fallback |
| `crates/core/audio/src/lib.rs` | AudioData loading and sync analysis |
| `crates/render/recording/examples/playback-test-runner.rs` | Playback benchmark runner |
...
**Changes Made**:
- Created `crates/render/editor/PLAYBACK-FINDINGS.md`
...
**Changes Made**:
- `crates/media/media/video-decode/src/avassetreader.rs`: Replaced `ffmpeg::format::input(&path).unwrap()` and `.ok_or(...).unwrap()` with `map_err()?` and `ok_or_else()?` for clean error propagation instead of panics
...
**Changes Made**:
- `crates/render/recording/src/studio_recording.rs`: System audio start_time now syncs to mic (or display) when drift >30ms, matching the existing camera/display sync pattern. Improves playback alignment.
...
**Changes Made**:
- `crates/media/media/video-decode/src/avassetreader.rs`: Single file open in KeyframeIndex::build (was opening the file twice - once for metadata, once for packet scan). Also caches pixel_format/width/height from the initial probe so pool decoders skip redundant FFmpeg opens.
- `crates/render/rendering/src/decoder/frame_converter.rs`: BGRA→RGBA conversion now processes 8 pixels (32 bytes) per loop iteration with direct indexed writes instead of per-pixel push(). Added fast path for RGBA when stride==width*4 (single memcpy instead of per-row copies).
