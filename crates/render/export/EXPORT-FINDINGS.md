...
The `effective_fps` tapering matches the H.264 encoder's `get_bitrate()` function in `crates/render/encoder-ffmpeg/src/video/h264.rs`. The `encoder_efficiency = 0.5` accounts for H.264 achieving better compression than the theoretical BPP * pixels calculation.
...
| File | Purpose |
|------|---------|
| `crates/render/export/src/lib.rs` | ExporterBase, builder pattern, export orchestration |
| `crates/render/export/src/mp4.rs` | MP4 export pipeline (NV12 render -> H.264 encode -> MP4 mux) |
| `crates/render/export/src/gif.rs` | GIF export pipeline (RGBA render -> gifski encode) |
| `crates/render/rendering/src/lib.rs` | `render_video_to_channel_nv12()` and `render_video_to_channel()` |
| `crates/render/encoder-ffmpeg/src/video/h264.rs` | H.264 encoder with BPP-based bitrate |
| `crates/render/encoder-ffmpeg/src/mux/mp4.rs` | MP4 muxer |
| `crates/render/encoder-gif/src/lib.rs` | GIF encoder wrapper |
...
| `crates/render/export/examples/export-benchmark-runner.rs` | Export benchmark runner |
...
- `crates/render/export/examples/export-benchmark-runner.rs`: Updated estimation functions to match backend
...
**Changes Made**:
- `crates/render/export/examples/export-benchmark-runner.rs`: New benchmark runner
- `crates/render/export/Cargo.toml`: Added chrono, tracing-subscriber dev-dependencies
- `crates/render/export/EXPORT-BENCHMARKS.md`: New benchmark history file
- `crates/render/export/EXPORT-FINDINGS.md`: New findings document
...
**Changes Made**:
- `crates/render/encoder-gif/src/lib.rs`: Replaced per-pixel `push(RGBA8::new(...))` double loop with bulk `slice::from_raw_parts` + `extend_from_slice`. When stride matches width*4, entire frame is cast in one operation. When stride differs, copies row-by-row via slice (O(height) instead of O(width*height)).
- `crates/render/encoder-ffmpeg/src/video/h264.rs`: Trimmed macOS encoder priority from `[h264_videotoolbox, h264_qsv, h264_nvenc, h264_amf, h264_mf, libx264]` to `[h264_videotoolbox, libx264]`. The 4 removed encoders never exist on macOS and just add failed init attempts.
- `crates/render/export/src/mp4.rs`: Increased NV12 render channel capacity from 2 to 8, allowing better pipeline overlap between GPU rendering and H.264 encoding. Memory cost: ~25MB at 1080p, ~100MB at 4K (acceptable for export).
...
3. The AVFoundation encoder (`crates/render/encoder-avfoundation/src/mp4.rs`) is only used for live recording, not export — a future IOSurface/CVPixelBuffer bridge from wgpu to VideoToolbox could eliminate the CPU NV12 copy in export, but that's a major architectural change
